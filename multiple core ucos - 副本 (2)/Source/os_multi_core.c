...
uint8_t OS_SendDataFinished(uint8_t devAddr)
{
		...
		/* 改变外核状态机模式 */
		status  = OS_Write(devAddr, SEND_DATA_FINISHED, NULL, 0);
		if(status)
		{
				return status;
		}
    ...
    OS_CPU_SR  cpu_sr = 0u;
		...
		if(OSCoreID == 0){
				...
				while(1){
						...
						OSSemQuery(DataTransferSem, &sem_data); // 获取信号量状态
						sprintf(str, ">>>>iq_sem:%u\n",sem_data.OSCnt);
...
void OS_SendData(uint8_t* buf, uint16_t size, uint8_t type)
{
#if OS_CRITICAL_METHOD == 3u                               /* Allocate storage for CPU status register     */
    OS_CPU_SR  cpu_sr = 0u;
#endif
